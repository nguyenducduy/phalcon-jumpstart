...
class NestedSet extends Behavior implements BehaviorInterface
{
    ...
    public function descendants($depth = null)
    {
        ...
        if ($this->hasManyRoots) {
            $query = $query->andWhere($this->rootAttribute.'='.$owner->{$this->primaryKey});
        }
    ...
    {
        return $this->addNode($target, $target->{$this->leftAttribute}+1, 1, $attributes, true);
    }
    ...
    {
        return $this->addNode($target, $target->{$this->rightAttribute}, 1, $attributes, true);
    }
    ...
    private function moveNode($target, $key, $levelUp)
    {
        ...
        if ($this->hasManyRoots && $owner->{$this->rootAttribute}!==$target->{$this->rootAttribute}) {
            ...
            foreach ([$this->leftAttribute,$this->rightAttribute] as $attribute) {
                ...
                foreach ($owner::find($condition) as $i) {
                    if ($i->update([$attribute=>$i->{$attribute}+$right-$left+1]) == false) {
                        $owner->getDI()->get('db')->rollback();
                        $this->ignoreEvent = false;
    ...
    private function shiftLeftRight($key, $delta)
    {
        ...
        foreach ([$this->leftAttribute, $this->rightAttribute] as $attribute) {
            ...
            $list = $ownerClass::find($condition);
            $this->ignoreEvent = true;
            if ($list->count() > 0) {
                foreach ($list as $item) {
                    $item->{$attribute} = $item->{$attribute} + $delta;
                    $item->save();
                }
            }
            $this->ignoreEvent = false;
/*
     ...
     * @throws \Phalcon\Mvc\Model\Exception
     */
    private function addNode($target, $key, $levelUp, $attributes, $ischildren = false)
    {
        $owner=$this->getOwner();
        ...
        if ($this->hasManyRoots) {
            $owner->{$this->rootAttribute} = $ischildren ? $target->{$this->primaryKey} : $target->{$this->rootAttribute};
        }
