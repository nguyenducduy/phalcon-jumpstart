 ...
 *
 */
class RESTController extends \Controllers\BaseController
{
	...
	/**
	 * Set when there is a 'order by' query parameter
	 * @var integer
	 */
	protected $orderby = null;

	/**
	 * Set when there is an 'order type' query parameter
	 * @var integer
	 */
	...
	protected function parseRequest($allowedFields){
		...
		$limit = $request->get('limit', null, null);
		$offset = $request->get('offset', null, null);
		$orderby = $request->get('orderby', null, null);
		$ordertype = $request->get('ordertype', null, null);

		// Set limits and offset, elsewise allow them to have defaults set in the Controller
		$this->limit = (int) $limit < 1 ? 10 : $limit;
		$this->offset = (int) $offset < 1 ? 1 : $offset;


		$this->orderby = $orderby == '' ? 'id' : $orderby;
		$this->ordertype = $ordertype == '' ? 'DESC' : $ordertype;
